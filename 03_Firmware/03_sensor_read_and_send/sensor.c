...
int32_t Sensor_GetTemperature(void)
{
	int32_t reading = 0;
...
bool Sensor_EnableSampling(uint8_t sensor_resolution)
{
    int res;
    uint16_t addr = TEMP_SENSOR_I2C_ADDR;
    uint16_t reg = TEMP_SENSOR_REG_CTRL_MEAS;
    uint8_t mode; //001 000 11 for 16 bit default
    int dev = 0;
    int flags = 0;
...
bool Sensor_Init(uint8_t sensor_resolution)
{
  ...
  bool ret = Sensor_EnableSampling(sensor_resolution);
